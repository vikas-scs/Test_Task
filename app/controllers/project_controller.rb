  ...
  def create                      
    ...
    begin
     Git.clone(params["github_url"],params["name"].to_s , :path => './public/')
      ...
      @projects.each do |project|
        if project.name == params["name"]                 #clone fields validation
          flash[:alert] = "name already exist"
          redirect_to new_project_path
          return
        end   
        if project.github_url == params["github_url"]
          flash[:alert] = "project alredy cloned with #{project.name}"
          redirect_to new_project_path
          return
        end   
      end 
      if !params["name"].present? && !params["github_url"].present?
    ...
    @limit = arr.length - 1 
      for i in 0..@limit
        ...
        arr1 = Array.new
        text_file = Array.new                                       
        if arr.keys[i] == "Ruby"                                    #checking Ruby version
      ...
      @arr = @project.name                                    #saving models,views and controller stats
        @ary1 = Dir["#{Rails.root}/public/#{@arr}/***/controllers/**/**.**"].map do |m|
        m.chomp('*.**').camelize.split("::").last
        end
      @mvc.controllers_list = @ary1
      @mvc.controllers_count = @ary1.count
        @ary2 = Dir["#{Rails.root}/public/#{@arr}/***/models/**/**.**"].map do |m|
        m.chomp('*.**').camelize.split("::").last
        end
      @mvc.models_count = @ary2.count
      @mvc.models_list = @ary2
        @ary3 = Dir["#{Rails.root}/public/#{@arr}/***/views/**/**.**"].map do |m|
        m.chomp('*.**').camelize.split("::").last
        end
